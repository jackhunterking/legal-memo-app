...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(
      `[Transcode] Downloaded audio file, size: ${audioBlob.size} bytes`
    );
    ...
    const job = (await createJobResponse.json()) as CloudConvertJobResponse;
    ...
    const getTaskResponse = await fetch(
      `https://api.cloudconvert.com/v2/tasks/${uploadTask.id}`,
      {
        headers: {
          Authorization: `Bearer ${cloudConvertApiKey}`,
        },
    ...
    const taskData = (await getTaskResponse.json()) as CloudConvertTaskResponse;
    ...
    for (const [key, value] of Object.entries(
      taskData.data.result.form.parameters
    )) {
    ...
    while (attempts < maxAttempts) {
      ...
      const statusData = (await statusResponse.json()) as CloudConvertJobResponse;
      ...
      console.log(
        `[Transcode] Job status: ${statusData.data.status}, attempt ${attempts + 1}`
      );
      attempts++;
    }
    ...
    console.log(
      `[Transcode] Successfully transcoded audio for meeting: ${meetingId}`
    );
