...
Deno.serve(async (req) => {
  ...
  try {
    const { meeting_id, task_title } = await req.json() as EnhanceTaskRequest
    ...
    const participants = meeting.ai_output?.meeting_overview?.participants || []
    const participantNames = participants
      .filter((p: any) => p.name && p.name !== 'Unknown')
      .map((p: any) => p.name)

    // Extract summary context
    const summaryContext = meeting.ai_output?.meeting_overview?.one_sentence_summary || 
                          'Meeting details not available'
    
    // Extract any existing follow-up actions for context
    const existingActions = meeting.ai_output?.follow_up_actions || []
    const actionsContext = existingActions
      .map((a: any) => `${a.action} (Owner: ${a.owner}, Deadline: ${a.deadline || 'none'})`)
      .join('\n')

    console.log(`[EnhanceTask] Found ${participantNames.length} participants`)
...
- Be conservative - prefer null over guessing`
        ...
        const openaiResponse = await fetch('https://api.openai.com/v1/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: 'system',
                content: 'You are a helpful assistant that analyzes legal meeting tasks and suggests task metadata. Always respond with valid JSON.',
              },
              {
                role: 'user',
                content: prompt,
        ...
        if (openaiResponse.ok) {
          ...
          enhancedTask = {
            ...
            suggested_deadline: suggestion.suggested_deadline === 'null' ? null : suggestion.suggested_deadline,
          }
        } else {
          console.warn('[EnhanceTask] OpenAI API error, using fallback logic')
        }
      } catch (aiError) {
    ...
    console.log(`[EnhanceTask] Enhanced task - Owner: ${enhancedTask.owner}, Deadline: ${enhancedTask.suggested_deadline}`)

    return new Response(
      JSON.stringify(enhancedTask),
    ...
    return new Response(
      JSON.stringify({
        error: error instanceof Error ? error.message : 'Task enhancement failed',
