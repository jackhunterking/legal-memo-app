...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[Migration] Found ${webmMeetings.length} WebM recordings to migrate`)

    const results: Array<{ meetingId: string, status: 'queued' | 'error', error?: string }> = []

    // Mark each meeting as transcoding and trigger the transcode function
    for (const meeting of webmMeetings) {
      try {
        ...
        const { error: invokeError } = await supabase.functions.invoke('transcode-audio', {
          body: { 
        ...
        if (invokeError) {
          console.error(`[Migration] Failed to trigger transcoding for ${meeting.id}:`, invokeError)
          results.push({ meetingId: meeting.id, status: 'error', error: invokeError.message })
          ...
          console.log(`[Migration] Successfully queued transcoding for ${meeting.id}`)
    ...
    console.log(`[Migration] Migration complete. Queued: ${successCount}, Errors: ${errorCount}`)
